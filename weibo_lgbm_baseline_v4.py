...
def fit_models_validate(
    ...
    train_ridge: csr_matrix,
    valid_ridge: csr_matrix,
    ...
    """
    v4：
    - LightGBM 仅吃 dense 数值分支（基础统计 + 近期统计 + 可选句向量）
    - Ridge 仅吃 sparse 文本分支（默认 TF-IDF）
    这样同时保留语义增强和文本稀疏优势，又避免 v3 的超大 hstack 占用。
    """
    lgb_models, ridge_models, lgb_preds, ridge_preds, best_iters = {}, {}, {}, {}, {}
    params = lightgbm_params(seed, num_threads=num_threads)
    train_num_values = train_num.values.astype(np.float32, copy=False)
    valid_num_values = valid_num.values.astype(np.float32, copy=False)
    for target in TARGETS:
        ...
        model.fit(
            train_num_values,
            y_tr,
            eval_set=[(valid_num_values, y_va)],
            eval_metric="l1",
            callbacks=[
        ...
        lgb_preds[target] = model.predict(valid_num_values, num_iteration=model.best_iteration_)
        best_iters[target] = int(model.best_iteration_ if model.best_iteration_ is not None else params["n_estimators"])
        if train_ridge.shape[1] == 0:
            ridge_models[target] = None
            ridge_preds[target] = lgb_preds[target].copy()
        else:
            ridge = Ridge(alpha=2.0, solver="lsqr", random_state=seed)
            ridge.fit(train_ridge, y_tr)
            ridge_models[target] = ridge
            ridge_preds[target] = ridge.predict(valid_ridge)
...
def fit_models_full(
    ...
    for target in TARGETS:
        ...
        y_tr = np.log1p(y_train[target].values.astype(np.float32))
        model = lgb.LGBMRegressor(**params)
        model.fit(train_num_values, y_tr)
        lgb_models[target] = model
        if train_ridge.shape[1] == 0:
            ridge_models[target] = None
        else:
...
def _validate_core(args, train_df: pd.DataFrame, valid_df: pd.DataFrame, output_dir: str, save_artifacts: bool = True) -> Dict[str, object]:
    ...
    print("[5/8] 构造 Ridge 文本特征矩阵...")
    X_train_ridge, scaler = make_sparse_text_features(
        vectorizer,
        train_df["content"],
        X_train_num,
        fit_scaler=True,
        fallback_to_numeric=not getattr(args, "use_tfidf", True),
    )
    X_valid_ridge, _ = make_sparse_text_features(
        vectorizer,
        valid_df["content"],
        X_valid_num,
        scaler=scaler,
    ...
    lgb_models, ridge_models, lgb_preds, ridge_preds, best_iters = fit_models_validate(
        X_train_num, X_valid_num, X_train_ridge, X_valid_ridge, y_train, y_valid, args.seed, args.num_threads
    )
    print("[7/8] 融合与后处理调优...")
...
def run_full(args) -> None:
    ...
    print("[5/7] 构造 Ridge 文本特征矩阵并训练全量模型...")
    X_train_ridge, scaler = make_sparse_text_features(
        vectorizer,
        train_df["content"],
        X_train_num,
    ...
    X_test_ridge, _ = make_sparse_text_features(
        vectorizer,
        test_df["content"],
        X_test_num,
        scaler=scaler,
    ...
    lgb_models, ridge_models = fit_models_full(X_train_num, X_train_ridge, train_df[TARGETS].copy(), args.seed, best_iterations, args.num_threads)
    print("[6/7] 预测测试集...")
    lgb_preds, ridge_preds = {}, {}
    X_test_num_values = X_test_num.values.astype(np.float32, copy=False)
    for target in TARGETS:
        lgb_preds[target] = lgb_models[target].predict(X_test_num_values)
        if ridge_models[target] is None:
            ridge_preds[target] = lgb_preds[target].copy()
        else:
            ridge_preds[target] = ridge_models[target].predict(X_test_ridge)
...
CONFIG = {
    ...
    # 输出目录
    "output_dir": "./outputs/weibo_baseline_v4",
    "submission_name": "submission.txt",
    # 文本特征：v3 支持两条文本支路
    "use_tfidf": True,
    # v4 默认把 TF-IDF 收缩一点，避免 CPU 内存压力过高
    ...
    "embedding_cache_dir": "./outputs/weibo_baseline_v4/embedding_cache",
    # v2 的近期历史窗口
    "recent_ks": [3, 5, 10],
    "recent_days": [7, 14, 30],
    # 训练参数
