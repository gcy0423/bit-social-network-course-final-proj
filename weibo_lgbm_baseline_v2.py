...
def make_infer_features_v2(
    ...
    for i, row in df.iterrows():
        ...
        if hist is None:
            ...
            end = int(np.searchsorted(hist["ts"], qts, side="left"))
...
def _validate_core(args, train_df: pd.DataFrame, valid_df: pd.DataFrame, output_dir: str, save_artifacts: bool = True) -> Dict[str, object]:
    ...
    X_train_sparse, scaler = make_sparse_text_features(vectorizer, train_df["content"], train_bundle.numeric_df, train_bundle.feature_cols, fit_scaler=True)
    X_valid_sparse, _ = make_sparse_text_features(vectorizer, valid_df["content"], valid_feat_df, train_bundle.feature_cols, scaler=scaler, fit_scaler=False)
...
def run_full(args) -> None:
    ...
    vectorizer = fit_text_vectorizer(train_df["content"], args.tfidf_max_features, args.tfidf_min_df)
    X_train_sparse, scaler = make_sparse_text_features(vectorizer, train_df["content"], train_bundle.numeric_df, train_bundle.feature_cols, fit_scaler=True)
    X_test_sparse, _ = make_sparse_text_features(vectorizer, test_df["content"], test_feat_df, train_bundle.feature_cols, scaler=scaler, fit_scaler=False)
...
CONFIG = {
    ...
    # 输出目录
    "output_dir": "./outputs/weibo_baseline_v2",
