...
def make_sparse_text_features(
    ...
    fallback_to_numeric: bool = True,
) -> Tuple[sparse.csr_matrix, Optional[StandardScaler]]:
    """
    v4 设计：
    - 默认只为 Ridge 文本支路构造 TF-IDF 稀疏特征，避免把 dense 数值特征和句向量再复制进 CSR，造成内存爆炸。
    - 仅当 vectorizer 为 None 且 fallback_to_numeric=True 时，才退回到数值特征 CSR。
    """
    if vectorizer is not None:
        X_text = vectorizer.transform(texts.fillna("").astype(str).tolist())
        return X_text.tocsr(), scaler
    if numeric_df is None or not fallback_to_numeric:
        n_rows = len(texts)
        return sparse.csr_matrix((n_rows, 0), dtype=np.float32), scaler
...
def lightgbm_binary_params(seed: int, num_threads: int) -> Dict[str, object]:
    return {
        "objective": "binary",
...
def default_best_iterations() -> Dict[str, Dict[str, int]]:
    return {t: {"clf": 500, "reg": 1200} for t in TARGETS}
...
def fit_models_validate(
    ...
    seed: int,
    num_threads: int,
) -> Tuple[Dict[str, object], Dict[str, object], Dict[str, np.ndarray], Dict[str, np.ndarray], Dict[str, Dict[str, int]]]:
    """
    v5：
    - 单一 dense 分支：基础统计 + 近期统计 + 可选句向量
    - 两阶段建模：
      1) LGBMClassifier 预测 y>0 的概率
      2) LGBMRegressor 仅在 y>0 样本上拟合 log1p(y)
    这样更贴近赛题的“零值很多、误报小正数也会掉分”的指标结构。
    """
    clf_models, reg_models, cls_probs, reg_preds_log, best_iters = {}, {}, {}, {}, {}
    reg_params = lightgbm_params(seed, num_threads=num_threads)
    cls_params = lightgbm_binary_params(seed, num_threads=num_threads)
    X_tr = train_num.values.astype(np.float32, copy=False)
    X_va = valid_num.values.astype(np.float32, copy=False)
    for target in TARGETS:
        y_tr_raw = y_train[target].values.astype(np.float32)
        y_va_raw = y_valid[target].values.astype(np.float32)
        y_tr_bin = (y_tr_raw > 0).astype(np.int32)
        y_va_bin = (y_va_raw > 0).astype(np.int32)
        ...
        cls_probs[target] = clf.predict_proba(X_va, num_iteration=clf.best_iteration_)[:, 1].astype(np.float32)
        best_clf_iter = int(clf.best_iteration_ if clf.best_iteration_ is not None else cls_params["n_estimators"])

        pos_mask_tr = y_tr_raw > 0
        pos_mask_va = y_va_raw > 0
        if int(pos_mask_tr.sum()) == 0:
            reg_models[target] = None
            reg_preds_log[target] = np.zeros(len(valid_num), dtype=np.float32)
            best_reg_iter = 0
        else:
            X_tr_pos = X_tr[pos_mask_tr]
            y_tr_pos = np.log1p(y_tr_raw[pos_mask_tr])
            reg = lgb.LGBMRegressor(**reg_params)
            callbacks = [lgb.log_evaluation(period=200)]
            fit_kwargs = {}
            if int(pos_mask_va.sum()) >= 50:
                X_va_pos = X_va[pos_mask_va]
                y_va_pos = np.log1p(y_va_raw[pos_mask_va])
                fit_kwargs = {
            ...
            reg.fit(X_tr_pos, y_tr_pos, **fit_kwargs)
            reg_models[target] = reg
            reg_preds_log[target] = reg.predict(X_va, num_iteration=getattr(reg, 'best_iteration_', None)).astype(np.float32)
            best_reg_iter = int(reg.best_iteration_ if getattr(reg, 'best_iteration_', None) is not None else reg_params["n_estimators"])
        best_iters[target] = {"clf": best_clf_iter, "reg": best_reg_iter}
    return clf_models, reg_models, cls_probs, reg_preds_log, best_iters
...
def fit_models_full(
    ...
    X_tr = train_num.values.astype(np.float32, copy=False)
    base_reg_params = lightgbm_params(seed, num_threads=num_threads)
    base_cls_params = lightgbm_binary_params(seed, num_threads=num_threads)
    for target in TARGETS:
        y_raw = y_train[target].values.astype(np.float32)
        y_bin = (y_raw > 0).astype(np.int32)

        cls_params = dict(base_cls_params)
        cls_params["n_estimators"] = int(best_iterations.get(target, {}).get("clf", 500))
        clf = lgb.LGBMClassifier(**cls_params)
        clf.fit(X_tr, y_bin)
        clf_models[target] = clf
        ...
        if int(pos_mask.sum()) == 0:
            ...
            reg_params = dict(base_reg_params)
            reg_params["n_estimators"] = int(best_iterations.get(target, {}).get("reg", 1200))
            reg = lgb.LGBMRegressor(**reg_params)
            reg.fit(X_tr[pos_mask], np.log1p(y_raw[pos_mask]))
            reg_models[target] = reg
    return clf_models, reg_models

def combine_two_stage_predictions(
    reg_preds_log: Dict[str, np.ndarray],
    cls_probs: Dict[str, np.ndarray],
    gate_thresholds: Dict[str, float],
    use_prob_scale: bool = False,
) -> np.ndarray:
    n = len(next(iter(cls_probs.values())))
    pred = np.zeros((n, 3), dtype=np.float32)
    for idx, target in enumerate(TARGETS):
        reg_log = reg_preds_log[target]
        p = cls_probs[target]
        value = np.clip(np.expm1(reg_log), 0.0, None).astype(np.float32)
        if use_prob_scale:
            value = value * p.astype(np.float32)
        value[p < float(gate_thresholds.get(target, 0.5))] = 0.0
        pred[:, idx] = value
    return pred
...
def tune_gate_thresholds(y_true: np.ndarray, reg_preds_log: Dict[str, np.ndarray], cls_probs: Dict[str, np.ndarray]) -> Dict[str, float]:
    thresholds = {"forward_count": 0.5, "comment_count": 0.6, "like_count": 0.5}
    grid = {
        "forward_count": [0.30, 0.40, 0.50, 0.60, 0.70, 0.80],
        "comment_count": [0.40, 0.50, 0.60, 0.70, 0.80, 0.90],
        "like_count": [0.30, 0.40, 0.50, 0.60, 0.70, 0.80],
    }
    base_pp = {t: {"scale": 1.0, "zero_thr": 0.0, "max_clip": 500.0} for t in TARGETS}
    best_score = official_score(y_true, apply_postprocess(combine_two_stage_predictions(reg_preds_log, cls_probs, thresholds), base_pp, int_mode="round"))
    for _ in range(2):
        for target in TARGETS:
            best_t = thresholds[target]
            for thr in grid[target]:
                trial = dict(thresholds)
                trial[target] = thr
                pred = combine_two_stage_predictions(reg_preds_log, cls_probs, trial)
...
def tune_postprocess(y_true: np.ndarray, pred_cont: np.ndarray) -> Tuple[Dict[str, Dict[str, float]], str, float]:
    config = {t: {"scale": 1.0, "zero_thr": 0.0, "max_clip": 500.0} for t in TARGETS}
    scale_grid = {t: [0.85, 0.95, 1.00, 1.05, 1.10] for t in TARGETS}
    thr_grid = {
        "forward_count": [0.0, 0.25, 0.5, 0.75, 1.0],
        "comment_count": [0.0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5],
        "like_count": [0.0, 0.25, 0.5, 0.75, 1.0],
    }
...
def _validate_core(args, train_df: pd.DataFrame, valid_df: pd.DataFrame, output_dir: str, save_artifacts: bool = True) -> Dict[str, object]:
    ...
    if sentence_encoder is not None:
        ...
        print("[3/6] 跳过预训练句向量分支...")
    y_train = train_df[TARGETS].copy()
    y_valid = valid_df[TARGETS].copy()
    print("[4/6] 训练两阶段模型...")
    clf_models, reg_models, cls_probs, reg_preds_log, best_iters = fit_models_validate(
        X_train_num, X_valid_num, y_train, y_valid, args.seed, args.num_threads
    )
    print("[5/6] 门控阈值与后处理调优...")
    y_valid_np = y_valid.values.astype(np.int32)
    gate_thresholds = tune_gate_thresholds(y_valid_np, reg_preds_log, cls_probs)
    pred_valid_cont = combine_two_stage_predictions(reg_preds_log, cls_probs, gate_thresholds)
    base_pp = {t: {"scale": 1.0, "zero_thr": 0.0, "max_clip": 500.0} for t in TARGETS}
    ...
    if save_artifacts:
        ...
        print("[6/6] 跳过单折模型落盘，仅保留评分摘要...")
    print("验证完成")
    print(f"基础分数(默认门控+默认后处理): {base_score:.6f}")
...
def run_cv(args) -> None:
    ...
    fold_results = []
    starts = list(args.cv_valid_starts)
    for fold_id, valid_start in enumerate(starts, start=1):
        print("\n" + "#" * 80)
        print(f"Fold {fold_id}: valid_start = {valid_start}")
        valid_start_dt = pd.to_datetime(valid_start)
        if fold_id < len(starts):
            valid_end_dt = pd.to_datetime(starts[fold_id])
        else:
            valid_end_dt = df["time"].max() + pd.Timedelta(seconds=1)
        train_df = df[df["time"] < valid_start_dt].copy().reset_index(drop=True)
        valid_df = df[(df["time"] >= valid_start_dt) & (df["time"] < valid_end_dt)].copy().reset_index(drop=True)
        if len(train_df) == 0 or len(valid_df) == 0:
        ...
        cfg["valid_end"] = str(valid_end_dt)
    ...
    print("[3/3] 现在的 CV 是按月不重叠切分，更适合判断线上泛化。")
def load_validate_config(path: Optional[str]) -> Dict[str, object]:
    if path is None or not os.path.exists(path):
        return {
            "best_iterations": default_best_iterations(),
            "gate_thresholds": {"forward_count": 0.5, "comment_count": 0.6, "like_count": 0.5},
            "postprocess": {t: {"scale": 1.0, "zero_thr": 0.0, "max_clip": 500.0} for t in TARGETS},
            "int_mode": "round",
        }
...
def run_full(args) -> None:
    ...
    if sentence_encoder is not None:
        ...
        print("[3/5] 跳过预训练句向量分支...")
    cfg = load_validate_config(args.validate_config)
    best_iterations = cfg.get("best_iterations", default_best_iterations())
    gate_thresholds = cfg.get("gate_thresholds", {"forward_count": 0.5, "comment_count": 0.6, "like_count": 0.5})
    postprocess = cfg.get("postprocess", {t: {"scale": 1.0, "zero_thr": 0.0, "max_clip": 500.0} for t in TARGETS})
    int_mode = cfg.get("int_mode", "round")
    print("[4/5] 训练全量两阶段模型并预测测试集...")
    clf_models, reg_models = fit_models_full(X_train_num, train_df[TARGETS].copy(), args.seed, best_iterations, args.num_threads)
    X_test_values = X_test_num.values.astype(np.float32, copy=False)
    cls_probs, reg_preds_log = {}, {}
    for target in TARGETS:
        cls_probs[target] = clf_models[target].predict_proba(X_test_values)[:, 1].astype(np.float32)
        reg_model = reg_models[target]
        if reg_model is None:
            reg_preds_log[target] = np.zeros(len(test_df), dtype=np.float32)
        else:
            reg_preds_log[target] = reg_model.predict(X_test_values).astype(np.float32)
    pred_test_cont = combine_two_stage_predictions(reg_preds_log, cls_probs, gate_thresholds)
...
CONFIG = {
    ...
    "seed": 42,
    "num_threads": 8,
    # full 模式复用 validate / cv 选出的配置；没有就填 None
